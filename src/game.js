...
function applySkillEffect(s,card,source,target,a,{reflected=false}={}){
 ...
 if(!reflected){const participants=[{playerId:source.id,category:'skillUse'}];if(card.target==='player'&&target.id!==source.id)participants.push({playerId:target.id,category:'skillTarget'});emitEmotion(s,{category:'skillUse',summary,participants});}
...
function useSkillCard(s,a){const p=s.players[s.turn];const overflowUse=a.overflowUse===true&&s.skillOverflow?.playerId===s.turn&&s.stage!=='finished';const normalUse=s.stage==='ready'&&!s.skillOverflow;if(!(normalUse||overflowUse)||!p?.human&&!p||p.bankrupt||p.skillUsedThisTurn||s.pendingSkill)return false;const card=skillCardById(a.cardId);if(!card||card.timing!=='active'||!SUPPORTED_ACTIVE_SKILLS.has(card.id)||!p.skillHand.includes(card.id))return false;const target=isLivePlayer(s,a.targetId)?s.players[a.targetId]:null;if(card.target==='player'&&(!target||target.id===p.id))return false;if(card.id==='slow'&&target.slowTurns>0)return false;if(card.id==='get-out'&&p.jail===0)return false;if(card.id==='teleport'&&(!Number.isInteger(a.targetPosition)||!LEGAL_TELEPORT_POSITIONS.includes(a.targetPosition)))return false;if(card.id==='land-swap'){const ownLot=s.lots[a.ownLotId],targetLot=s.lots[a.targetLotId];if(!ownLot||!targetLot||ownLot.owner!==p.id||targetLot.owner===p.id||!TILES[a.ownLotId]||!TILES[a.targetLotId]||TILES[a.ownLotId].type!=='property'||TILES[a.targetLotId].type!=='property'||!isLivePlayer(s,targetLot.owner))return false;[ownLot.owner,targetLot.owner]=[targetLot.owner,ownLot.owner];}
 ...
 if(card.target==='player'){const reactions=skillReactionOptions(s,target);if(reactions.counter||reactions.reflect){s.pendingSkill={sourceId:p.id,targetId:target.id,cardId:card.id,targetPosition:Number.isInteger(a.targetPosition)?a.targetPosition:null,ownLotId:a.ownLotId,targetLotId:a.targetLotId};note(s,`${p.name}使用${card.name}，等待${target.name}回應。`,'skill');return true;}}
