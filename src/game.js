...
export const SKILL_CARDS=[
  {id:'trap',name:'陷害卡',type:'control',timing:'active',icon:'⛓️',summary:'指定玩家進入監獄',target:'player',revengeable:true,description:'將一名玩家送進監獄，該玩家會移到監獄位置並暫停下一回合。',usage:'在自己的回合、擲骰前選擇一名目標玩家使用。',restriction:'只能指定仍在場的其他玩家；目標若持有反制卡、反彈卡或復仇卡，可以在效果結算前回應。'},
  {id:'slow',name:'龜速卡',type:'control',timing:'active',icon:'🐌',summary:'目標 3 回合最多走 1 格',target:'player',revengeable:true,description:'使目標玩家接下來 3 個自己的回合，每回合最多前進 1 格，但仍會觸發抵達格子的效果。',usage:'在自己的回合、擲骰前選擇一名玩家使用，也可以指定自己。',restriction:'目標已有龜速效果時不能重複套用；指定其他玩家時，對方可使用反制卡、反彈卡或復仇卡。'},
  {id:'stay',name:'停留卡',type:'control',timing:'active',icon:'⏸️',summary:'目標下回合原地重觸格子效果',target:'player',revengeable:true,description:'使目標玩家下一回合擲骰後停留在原地一次，重新觸發目前格子的效果。',usage:'在自己的回合、擲骰前選擇一名玩家使用，也可以指定自己。',restriction:'不會因停留領取起點獎勵；入獄規則優先；指定其他玩家時，對方可使用反制卡、反彈卡或復仇卡。'},
 ...
 {id:'position-swap',name:'位置交換卡',type:'movement',timing:'active',icon:'⇄',summary:'交換兩名玩家位置',target:'player',revengeable:true,description:'將自己與指定玩家目前在棋盤上的位置互換。',usage:'在自己的回合、擲骰前選擇一名其他玩家使用。',restriction:'只能指定仍在場的其他玩家；交換位置不會重新觸發抵達格子效果，目標若持有反制卡、反彈卡或復仇卡，可以在效果結算前回應。'},
 ...
 {id:'revenge',name:'復仇卡',type:'defense',timing:'reaction',icon:'⚔️',summary:'讓指定者承受同樣效果',description:'當自己被指定入獄、龜速、停留或交換類技能時，讓原本的指定者承受同樣效果；交換類技能會交換後再交換回來。',usage:'其他玩家對自己使用可復仇的單一目標技能、且效果尚未結算時，在反應提示中選擇「使用復仇卡」。',restriction:'每個技能事件最多復仇一次；只能對可復仇的入獄、龜速、停留與交換類技能使用。'},
 ...
 {id:'land-swap',name:'換地卡',type:'control',timing:'active',icon:'🏠',summary:'交換兩塊地產所有權',target:'land',revengeable:true,description:'將自己擁有的一塊普通地產，與其他存活玩家擁有的一塊普通地產交換所有權。',usage:'在自己的回合、擲骰前選擇自己的土地與對方土地後使用。',restriction:'雙方都必須是普通地產；不能選特殊格、未出售地產或已破產玩家的地產。交換後保留原有房屋／旅館等級，不會觸發抵達效果；目標可使用復仇卡抵銷交換。'},
 {id:'demolish',name:'拆屋卡',type:'control',timing:'active',icon:'🔨',summary:'拆除一間房屋或降級旅館',target:'land',description:'指定一塊已有建築的普通地產，拆除一間房屋；若目標是旅館，則降級為 4 間房屋。',usage:'在自己的回合、擲骰前選擇任一存活玩家的建築地產使用。',restriction:'目標必須已有房屋或旅館；旅館降級需要銀行有 4 間可用房屋。'},
 {id:'building-swap',name:'換屋卡',type:'control',timing:'active',icon:'🏗️',summary:'交換兩塊地產的建築等級',target:'land',revengeable:true,description:'交換自己與其他存活玩家各一塊普通地產的房屋／旅館等級，土地所有權不變。',usage:'在自己的回合、擲骰前選擇自己的土地與對方土地後使用。',restriction:'雙方都必須是普通地產與存活玩家持有；只交換建築等級，不交換所有權，也不重新觸發抵達效果；目標可使用復仇卡抵銷交換。'}
];
...
export const SUPPORTED_ACTIVE_SKILLS=new Set(['trap','slow','stay','teleport','get-out','rent-shield','rent-double','position-swap','birthday','subsidy','redraw','land-swap','demolish','building-swap']);
...
function skillReactionOptions(s,target,card){return {counter:target.skillHand.includes('counter'),reflect:card.target==='player'&&target.skillHand.includes('reflect'),revenge:!!card.revengeable&&target.skillHand.includes('revenge')};}
export function canUseSkillCard(s,a={}){
 ...
 const targetId=card.target==='land'?s.lots[a.targetLotId]?.owner:a.targetId;
 const target=isLivePlayer(s,targetId)?s.players[targetId]:null;
 const selfTargetAllowed=card.id==='slow'||card.id==='stay';
 ...
 if(card.id==='stay'&&target.stayTurns>0)return false;
 ...
 if(card.id==='demolish'){
  const targetLot=s.lots[a.targetLotId],bank=availableBuildings(s);
  if(!targetLot||!TILES[a.targetLotId]||TILES[a.targetLotId].type!=='property'||!isLivePlayer(s,targetLot?.owner)||targetLot.level<=0||(targetLot.level===5&&s.limitBuildings!==false&&bank.houses<4))return false;
 }
 if(card.id==='land-swap'||card.id==='building-swap'){
...
function applySkillEffect(s,card,source,target,a,{reflected=false,skipLog=false}={}){
 if(card.id==='trap'){target.pos=8;target.jail=1;}
 if(card.id==='slow')target.slowTurns=3;
 if(card.id==='stay')target.stayTurns=1;
 ...
 if(card.id==='demolish'){const lot=s.lots[a.targetLotId];if(lot.level===5){lot.level=4;if(s.limitBuildings!==false){s.bank=s.bank||availableBuildings(s);s.bank.hotels++;s.bank.houses-=4;}}else{lot.level--;if(s.limitBuildings!==false){s.bank=s.bank||availableBuildings(s);s.bank.houses++;}}}
 if(card.id==='land-swap'){const ownLot=s.lots[a.ownLotId],targetLot=s.lots[a.targetLotId];[ownLot.owner,targetLot.owner]=[targetLot.owner,ownLot.owner];}
 if(card.id==='building-swap'){const ownLot=s.lots[a.ownLotId],targetLot=s.lots[a.targetLotId];[ownLot.level,targetLot.level]=[targetLot.level,ownLot.level];}
 ...
 if(skipLog)return;
 const targetText=card.id==='demolish'?`，目標為${TILES[a.targetLotId].name}`:card.target==='player'?`，目標為${target.name}`:card.target==='land'?`，交換${TILES[a.ownLotId].name}與${TILES[a.targetLotId].name}`:'';
 const summary=`${reflected?'反彈後，':''}${source.name}使用${card.name}${targetText}。`;note(s,summary,'skill');
 if(!reflected){const participants=[{playerId:source.id,category:'skillUse'}];if((card.target==='player'||card.target==='land')&&target?.id!==source.id)participants.push({playerId:target.id,category:card.id==='land-swap'||card.id==='building-swap'?'skillLandSwapTarget':'skillTarget'});if(card.id==='birthday'){for(const other of s.players)if(other.id!==source.id&&!other.bankrupt)participants.push({playerId:other.id,category:'skillTarget'});}emitEmotion(s,{category:'skillUse',summary,participants});}
 if(card.id==='land-swap')note(s,`${TILES[a.ownLotId].name}與${TILES[a.targetLotId].name}的所有權已交換。`,'skill');
 if(card.id==='building-swap')note(s,`${TILES[a.ownLotId].name}與${TILES[a.targetLotId].name}的建築等級已交換。`,'skill');
}
function applyRevengeEffect(s,card,source,target,a){
 const exchangeCard=['position-swap','land-swap','building-swap'].includes(card.id);
 if(exchangeCard){applySkillEffect(s,card,source,target,a,{skipLog:true});applySkillEffect(s,card,target,source,a,{skipLog:true});}
 else applySkillEffect(s,card,source,source,a,{skipLog:true});
 const summary=`${target.name}使用復仇卡，讓${source.name}承受${card.name}的效果。`;note(s,summary,'skill');emitEmotion(s,{category:'skillRevenge',summary,participants:[{playerId:target.id,category:'skillReflect'},{playerId:source.id,category:'skillTarget'}]});
}
function resolvePendingSkill(s,response='pass'){
 ...
 if(response==='revenge'&&card.revengeable&&discardSkillCard(s,target,'revenge')&&isLivePlayer(s,source.id)){s.pendingSkill=null;applyRevengeEffect(s,card,source,target,{targetPosition:pending.targetPosition,ownLotId:pending.ownLotId,targetLotId:pending.targetLotId});if(!['position-swap','land-swap','building-swap'].includes(card.id))applySkillEffect(s,card,source,target,{targetPosition:pending.targetPosition,ownLotId:pending.ownLotId,targetLotId:pending.targetLotId});return true;}
 ...
 function useSkillCard(s,a){const p=s.players[s.turn];if(!canUseSkillCard(s,a))return false;const card=skillCardById(a.cardId),targetId=card.target==='land'?s.lots[a.targetLotId]?.owner:a.targetId,target=isLivePlayer(s,targetId)?s.players[targetId]:null;
 ...
 if(target&&target.id!==p.id&&card.revengeable){const reactions=skillReactionOptions(s,target,card);if(reactions.counter||reactions.reflect||reactions.revenge){s.pendingSkill={sourceId:p.id,targetId:target.id,cardId:card.id,targetPosition:Number.isInteger(a.targetPosition)?a.targetPosition:null,ownLotId:a.ownLotId,targetLotId:a.targetLotId};const summary=`${p.name}使用${card.name}，指定${target.name}，等待回應。`;note(s,summary,'skill');emitEmotion(s,{category:'skillUse',summary,participants:[{playerId:p.id,category:'skillUse'},{playerId:target.id,category:'skillTarget'}]});return true;}}
 applySkillEffect(s,card,p,target||p,a);return true;
}
...
export function reducer(state,a){
 ...
 switch(a.type){
  case 'SET_DICE_COUNT':if(s.stage!=='ready'||!p.human||p.bankrupt||p.jail||![1,2].includes(a.count))return state;p.diceCount=a.count;break;
   case 'ROLL':if(s.stage!=='ready'||p.bankrupt||s.pendingSkill)return state;if(p.jail){p.jail--;p.stayTurns=0;s.stage='end';note(s,`${p.name}在監獄休息一回合，下回合恢復行動。`);break;}if(!Array.isArray(a.dice)||a.dice.length!==(p.diceCount??2)||!a.dice.every(d=>Number.isInteger(d)&&d>=1&&d<=6))return state;s.dice=[...a.dice];const rolled=a.dice.reduce((sum,d)=>sum+d,0),slowed=p.slowTurns>0,staying=p.stayTurns>0;p.slowTurns=Math.max(0,p.slowTurns-(slowed?1:0));p.stayTurns=0;s.event=null;s.eventIndex=a.eventIndex;if(staying){s.remaining=0;s.stage='moving';note(s,`${p.name}擲出 ${a.dice.join(' + ')}，停留卡生效，原地重新觸發格子效果。`,'dice');const stayingFundSkill=TILES[p.pos].type==='fund'?drawSkillCard(s,p.id,'停留於城市基金格，'):null;land(s,s.eventIndex);if(stayingFundSkill&&s.event)s.event={...s.event,skillCardId:stayingFundSkill.id,skillCardName:stayingFundSkill.name};}else{s.remaining=slowed?Math.min(1,rolled):rolled;s.stage='moving';note(s,`${p.name}擲出 ${a.dice.join(' + ')}，前進 ${s.remaining} 格。${slowed?'龜速效果生效，本回合最多前進 1 格。':''}`,'dice');}break;
  ...
  case 'USE_SKILL_CARD':if(!useSkillCard(s,a))return state;break;
   case 'RESPOND_SKILL':if(!s.pendingSkill||a.playerId!==s.pendingSkill.targetId||!['counter','reflect','revenge','pass'].includes(a.response)||!resolvePendingSkill(s,a.response))return state;break;
