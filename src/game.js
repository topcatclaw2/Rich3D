...
export const canBuild=(s,id,p=s.turn)=>{const t=TILES[id],l=s.lots[id],bank=availableBuildings(s);if(!t||t.type!=='property'||!l||l.owner!==p||s.turn!==p||s.stage!=='end'||s.selected!==id||!s.buildAvailable||s.buildUsed||l.level>=5||s.players[p].cash<buildCost(t,s))return false;return l.level<4?bank.houses>0:bank.hotels>0;};
...
export const SUPPORTED_ACTIVE_SKILLS=new Set(['trap','slow','get-out','rent-shield','rent-double','position-swap','birthday','subsidy','redraw','land-swap']);
...
function useSkillCard(s,a){const p=s.players[s.turn];const overflowUse=a.overflowUse===true&&s.skillOverflow?.playerId===s.turn&&s.stage!=='finished';const normalUse=s.stage==='ready'&&!s.skillOverflow;if(!(normalUse||overflowUse)||!p?.human||p.bankrupt||p.skillUsedThisTurn)return false;const card=skillCardById(a.cardId);if(!card||card.timing!=='active'||!SUPPORTED_ACTIVE_SKILLS.has(card.id)||!p.skillHand.includes(card.id))return false;const target=isLivePlayer(s,a.targetId)?s.players[a.targetId]:null;if(card.target==='player'&&(!target||target.id===p.id))return false;if(card.id==='slow'&&target.slowTurns>0)return false;if(card.id==='get-out'&&p.jail===0)return false;if(card.id==='land-swap'){const ownLot=s.lots[a.ownLotId],targetLot=s.lots[a.targetLotId];if(!ownLot||!targetLot||ownLot.owner!==p.id||targetLot.owner===p.id||!TILES[a.ownLotId]||!TILES[a.targetLotId]||TILES[a.ownLotId].type!=='property'||TILES[a.targetLotId].type!=='property'||!isLivePlayer(s,targetLot.owner))return false;[ownLot.owner,targetLot.owner]=[targetLot.owner,ownLot.owner];}
 if(card.id==='trap'){target.pos=8;target.jail=1;}
 if(card.id==='slow')target.slowTurns=3;
 if(card.id==='get-out')p.jail=0;
 if(card.id==='rent-shield')p.rentShield=1;
 if(card.id==='rent-double')p.rentMultiplier=2;
 if(card.id==='position-swap'){[p.pos,target.pos]=[target.pos,p.pos];}
 if(card.id==='birthday'){const amount=inflationAmount(s,300);for(const other of s.players)if(other.id!==p.id&&!other.bankrupt)pay(s,other.id,amount,p.id,'skill');}
 if(card.id==='subsidy'){const amount=inflationAmount(s,800);p.cash+=amount;emitEmotion(s,{category:'windfall',summary:`${p.name}使用城市補助卡，獲得 ${money(amount)}。`,participants:[{playerId:p.id,amountDelta:amount}]});}
 if(card.id==='redraw'){discardSkillCard(s,p,card.id);p.skillUsedThisTurn=true;drawSkillCard(s,p.id);if(s.skillOverflow?.playerId===p.id&&p.skillHand.length<=MAX_SKILL_HAND)s.skillOverflow=null;note(s,`${p.name}使用幸運重抽卡，替換一張手牌。`,'skill');return true;}
 discardSkillCard(s,p,card.id);p.skillUsedThisTurn=true;if(s.skillOverflow?.playerId===p.id&&p.skillHand.length<=MAX_SKILL_HAND)s.skillOverflow=null;const targetText=card.target==='player'?`，目標為${target.name}`:card.target==='land'?`，交換${TILES[a.ownLotId].name}與${TILES[a.targetLotId].name}`:'';note(s,`${p.name}使用${card.name}${targetText}。`,'skill');if(card.id==='land-swap')note(s,`${TILES[a.ownLotId].name}與${TILES[a.targetLotId].name}的所有權已交換。`,'skill');return true;}
...
function land(s,eventIndex){
 ...
 if(t.type==='property'){
  const l=s.lots[t.id];
  if(!l){const price=propertyPrice(s,t);s.stage=p.cash>=price?'decision':'end';note(s,`${p.name}抵達${t.name}，${p.cash>=price?'可以購買這塊地產。':'現金不足以購買。'}`);}
  ...
  const skill=t.type==='chance'?drawSkillCard(s,p.id,'機會事件中，'):null;if(skill)s.event={...s.event,skillCardId:skill.id,skillCardName:skill.name};
...
export function reducer(state,a){
 ...
 switch(a.type){
  ...
  case 'STEP':if(s.stage!=='moving')return state;p.pos=(p.pos+1)%32;if(p.pos===0){const salary=inflationAmount(s,2000);p.cash+=salary;note(s,`${p.name}通過起點，領取 ${money(salary)}。`,'salary');emitEmotion(s,{category:'salary',summary:`${p.name}經過起點，獲得 ${money(salary)}。`,participants:[{playerId:p.id,amountDelta:salary}]});}s.remaining--;if(TILES[p.pos].type==='fund')drawSkillCard(s,p.id,s.remaining===0?'抵達城市基金格，':'經過城市基金格，');if(s.remaining===0)land(s,s.eventIndex);break;
