...
const Board=forwardRef(function Board({game,onSelect,onError},ref){
 ...
 useEffect(()=>{
  ...
  const tiles=[];const tileBases=[];const lotGroups=[];
  ...
  function animate(){frame=requestAnimationFrame(animate);const now=performance.now();const dt=Math.min((now-previousTime)/1000,.05);previousTime=now;elapsed+=dt;const s=current.current;
   ...
   const lots=JSON.stringify(s.lots);if(lastLots!==lots){lastLots=lots;for(const t of TILES){const g=lotGroups[t.id];while(g.children.length)g.remove(g.children[0]);const l=s.lots[t.id];const ownerColor=l?s.players[l.owner]?.color||COLORS[l.owner]:null;tileBases[t.id].material.color.set(ownerColor||'#fffaf0');if(l){
    // Make ownership unmistakable: a four-sided colored frame sits above the tile face.
    box(g,1.82,.07,.1,ownerColor,0,.1,.89,.02);box(g,1.82,.07,.1,ownerColor,0,.1,-.89,.02);box(g,.1,.07,1.62,ownerColor,.86,.1,0,.02);box(g,.1,.07,1.62,ownerColor,-.86,.1,0,.02);
    box(g,1.75,.055,.13,ownerColor,0,0,.84,.015);if(l.level===5){box(g,.62,.72,.58,ownerColor,0,.36,-.18,.04);box(g,.72,.08,.68,'#f8f0d8',0,.76,-.18,.02);const roof=new THREE.Mesh(new THREE.ConeGeometry(.43,.28,4),mat('#d2a451'));roof.position.set(0,.96,-.18);roof.rotation.y=Math.PI/4;roof.castShadow=true;g.add(roof);box(g,.18,.18,.03,'#e7b64d',0,.42,-.49,.01);}else{const positions=[[-.3,-.35],[.3,-.35],[-.3,.05],[.3,.05]];for(let n=0;n<l.level;n++){const [hx,hz]=positions[n];box(g,.33,.32,.34,ownerColor,hx,.2,hz,.02);const roof=new THREE.Mesh(new THREE.ConeGeometry(.3,.19,4),mat('#faf0d7'));roof.position.set(hx,.45,hz);roof.rotation.y=Math.PI/4;roof.castShadow=true;g.add(roof);}}}}}
