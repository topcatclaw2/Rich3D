...
const Board=forwardRef(function Board({game,onSelect,onError},ref){
 ...
 useEffect(()=>{
  ...
  const tiles=[];const tileBases=[];const lotGroups=[];const priceLabelRefreshers=[];
  const ownerBadges=new Map();
  function ownerBadge(owner,color){
   const key=`${owner}:${color}`;
   if(!ownerBadges.has(key)){
    const tex=labelTexture((ctx,w,h)=>{
     ctx.clearRect(0,0,w,h);ctx.fillStyle=color;ctx.beginPath();ctx.roundRect(8,8,w-16,h-16,24);ctx.fill();
     ctx.strokeStyle='#fffaf0';ctx.lineWidth=8;ctx.stroke();
     ctx.fillStyle='#fffaf0';ctx.beginPath();ctx.arc(w/2,h/2,43,0,Math.PI*2);ctx.fill();
     ctx.fillStyle='#294237';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='bold 72px Arial';ctx.fillText(String(owner+1),w/2,h/2+3);
    },192,144);
    ownerBadges.set(key,new THREE.SpriteMaterial({map:tex,toneMapped:false,depthTest:false,depthWrite:false}));
   }
   return ownerBadges.get(key);
  }
  for(const t of TILES){const [x,z]=tilePosition(t.id);const g=new THREE.Group();g.position.set(x,.44,z);scene.add(g);const b=box(g,1.94,.14,1.94,'#fffaf0',0,0,0,.035);b.userData.tile=t.id;tiles.push(b);tileBases[t.id]=b;
   const initialIndex=current.current.priceIndex||1;
   const drawTileLabel=(ctx,w,h,index)=>{ctx.fillStyle='#fcf9f0';ctx.fillRect(0,0,w,h);if(t.type==='property'){ctx.fillStyle=t.color;ctx.fillRect(0,0,w,82);}ctx.fillStyle='#294237';ctx.textAlign='center';ctx.font=`bold 76px ${cjkFont}`;ctx.fillText(t.name,w/2,t.type==='property'?180:140);if(t.type==='property'){ctx.font='60px Arial';ctx.fillText(money(Math.round(t.price*index)),w/2,290);ctx.fillStyle='#8c998e';ctx.font='26px sans-serif';ctx.fillText('CITY PROPERTY',w/2,410);}else{ctx.font='bold 135px Arial';ctx.fillStyle=t.type==='chance'?'#ba9270':'#648473';ctx.fillText(icons[t.type],w/2,320);ctx.fillStyle='#728375';ctx.font=`30px ${cjkFont}`;ctx.fillText(t.type==='start'?`+ ${money(Math.round(2000*index))}`:t.type==='tax'?'城市稅收':t.type==='park'?'歇一會兒':t.type==='chance'?'好運降臨':t.type==='fund'?'城市生活':'JUST VISITING',w/2,420);}};
   const tex=labelTexture((ctx,w,h)=>drawTileLabel(ctx,w,h,initialIndex));
   if(t.type==='property'||t.type==='start')priceLabelRefreshers.push(index=>{const ctx=tex.image.getContext('2d');drawTileLabel(ctx,tex.image.width,tex.image.height,index);tex.needsUpdate=true;});
   ...
   const props=new THREE.Group();props.position.set(x,.58,z);props.rotation.y=t.id<8?0:t.id<16?Math.PI/2:t.id<24?Math.PI:-Math.PI/2;scene.add(props);lotGroups[t.id]=props;
  }
  ...
  function animate(){frame=requestAnimationFrame(animate);const now=performance.now();const dt=Math.min((now-previousTime)/1000,.05);previousTime=now;elapsed+=dt;const s=current.current;
   ...
   const lots=JSON.stringify([s.lots,s.players.map(p=>p.color)]);if(lastLots!==lots){lastLots=lots;for(const t of TILES){const g=lotGroups[t.id];while(g.children.length){const child=g.children[0];if(child.userData.disposableGeometry)child.geometry.dispose();g.remove(child);}const l=s.lots[t.id];const ownerColor=l?s.players[l.owner]?.color||COLORS[l.owner]:null;tileBases[t.id].material=mat(ownerColor||'#fffaf0');if(l){
    // Persistent camera-facing owner number, at the outer corner away from buildings.
