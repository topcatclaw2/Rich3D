...
export function validGame(s){
 return !!(s?.version===1 && Array.isArray(s.players)&&s.players.length>=2&&s.players.length<=4&&s.players.every((p,i)=>p.id===i&&typeof p.name==='string'&&Number.isFinite(p.cash)&&p.cash>=0&&Number.isInteger(p.pos)&&p.pos>=0&&p.pos<32&&[0,1].includes(p.jail)&&typeof p.bankrupt==='boolean'&&(p.diceCount===undefined||[1,2].includes(p.diceCount)))&&s.lots&&typeof s.lots==='object'&&!Array.isArray(s.lots)&&Object.entries(s.lots).every(([id,l])=>Number.isInteger(+id)&&+id>0&&+id<32&&![3,6,8,11,14,16,19,22,24,27,30].includes(+id)&&l&&Number.isInteger(l.owner)&&l.owner>=0&&l.owner<s.players.length&&Number.isInteger(l.level)&&l.level>=0&&l.level<=5)&&Number.isInteger(s.turn)&&s.turn>=0&&s.turn<s.players.length&&Number.isInteger(s.round)&&s.round>=1&&(s.maxRounds===undefined||s.maxRounds===null||Number.isInteger(s.maxRounds)&&s.maxRounds>=1)&&((s.maxRounds===undefined||s.maxRounds===null)||s.round<=s.maxRounds+1)&&(s.inflationRate===undefined||Number.isInteger(s.inflationRate)&&s.inflationRate>=0&&s.inflationRate<=10)&&(s.priceIndex===undefined||Number.isFinite(s.priceIndex)&&s.priceIndex>=1&&s.priceIndex<=10)&&['ready','moving','decision','end','finished'].includes(s.stage)&&Array.isArray(s.dice)&&[1,2].includes(s.dice.length)&&s.dice.every(d=>Number.isInteger(d)&&d>=1&&d<=6)&&Number.isInteger(s.remaining)&&s.remaining>=0&&s.remaining<=12&&(s.stage!=='moving'||s.remaining>0&&Number.isInteger(s.eventIndex)&&s.eventIndex>=0&&s.eventIndex<8)&&Array.isArray(s.log)&&s.log.every(e=>e&&typeof e.text==='string'&&typeof e.kind==='string')&&typeof s.notice==='string'&&(s.event===null||s.event&&typeof s.event.title==='string'&&(s.event.amount===undefined||Number.isFinite(s.event.amount)))&&(s.stage!=='finished'||Number.isInteger(s.winner)&&s.winner>=0&&s.winner<s.players.length)&&(s.bank===undefined||(Number.isInteger(s.bank.houses)&&s.bank.houses>=0&&Number.isInteger(s.bank.hotels)&&s.bank.hotels>=0))&&(s.buildAvailable===undefined||typeof s.buildAvailable==='boolean')&&(s.buildUsed===undefined||typeof s.buildUsed==='boolean'));
}
